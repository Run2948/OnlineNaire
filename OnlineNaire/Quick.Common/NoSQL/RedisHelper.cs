/* ==============================================================================
* 命名空间：Quick.Common.NoSQL
* 类 名 称：RedisHelper
* 创 建 者：Qing
* 创建时间：2018-12-18 14:10:41
...
namespace Quick.Common.NoSQL
{
    ...
    public class RedisHelper : Disposable
    {
        ...
        public RedisHelper(int db = 0) : this(null, db)
        {
        ...
        public RedisHelper(string readWriteHosts, int db = 0)
        {
            Db = db;
        ...
        /// <param name="db">数据库的编号</param>
        /// <param name="_"></param>
        private RedisHelper(string readWriteHosts, int db, int _)
        {
            Db = db;
        ...
        {
            IEnumerable<RedisKey> redisKeys = LikeKeys(pattern);
            return Do(db => db.KeyDelete(redisKeys.ToArray()));
        }
        ...
        /// <returns></returns>
        public Dictionary<string, string> GetAllKeys(string pattern = "*")
        {
        ...
        {
            return To(server => server.Keys(database: Db, pattern: pattern));
        }
        ...
        private T To<T>(Func<IServer, T> func)
        {
